...
export default function Card({ result }: CardProps) {
    ...
    const movieTitle = result.title || result.name || result.original_title || "No Title Available";
  
    // Limit overview to 5 lines (approx. 300 characters)
    const maxOverviewLength = 300; 
    const limitedOverview = result.overview.length > maxOverviewLength
      ? `${result.overview.slice(0, maxOverviewLength)}...`
      : result.overview;
  
    // Limit title to 1 line
    const limitedTitle = movieTitle.length > 100 
      ? `${movieTitle.slice(0, 100)}...` 
      : movieTitle;
  
    // Convert genres to a string (single line)
    const limitedGenres = genreNames.length > 100 
      ? `${genreNames.slice(0, 100)}...` 
      : genreNames;
    ...
    return (
      <div className="group cursor-pointer sm:hover:shadow-slate-400 sm:shadow-md rounded-lg sm:border sm:border-slate-400 sm:m-4 p-4 transition-shadow duration-200 w-full max-w-md"> {/* Make the card wider */}
